...
class Keychain:
    ...
    def store_keys(pub_key: str, priv_key: str) -> bool:
        try:
            keyring.set_password("me.theosmithscott", pub_key, priv_key)
    ...
    def fetch_priv(pub_key: str):
        try:
            return keyring.get_password("me.theosmithscott", pub_key)
        except RuntimeError as error:
            return False
    ...
    def add_auth(username: str, password_hash: str) -> bool:
        try:
            keyring.set_password("me.theosmithscott", username, password_hash)
            return True
        except RuntimeError as error:
            print(error)
    ...
    def fetch_auth(username: str) -> str:
        try:
            return keyring.get_password("me.theosmithscott", username)
        except RuntimeError as error:
            print(error)